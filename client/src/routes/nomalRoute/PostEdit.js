...
// ! 얘같은 경우는 인증된 사람만 글을 수정할 수 있도록 하고싶은데
// ! 만일 주소창에 글의 고유 id 값 뒤에 /edit만 넣어서 글을 수정하도록 냅두면 보안의 의미가 없다
// ! 그래서 Router.js에서 protectedRoute(개발자가 만든 이름)로 진행을 하려고 한다
const PostEdit = () => {
    ...
    const onSubmit = async (e) => {
        ...
        const { title, contents, fileUrl } = form; // state에 저장된 form
        const token = localStorage.getItem("token");
        const id = postDetail._id; // * post의 id
    ...
    // * postDetail의 title과 contents와 fileUrl이 달라졌다고 하면
    // * state를 저장하라고 작성한 useEffect
    useEffect(() => {
        setValues({
            ...
            {isAuthenticated ? (
                ...
                <Form onSubmit={onSubmit}>
                    <FormGroup className="mb-3">
                        ...
                        <Input
                            defaultValue={postDetail.title} // * Input의 초기값 설정
                            type="text"
                            name="title"
                            id="title"
                    ...
                    <FormGroup className="mb-3">
                        ...
                        <CKEditor
                            ...
                            data={postDetail.contents} // * CKEditor의 초기값 설정
                            onInit={Myinit}
